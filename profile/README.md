...
마이크로 서비스의 특징은 바로 하나의 마이크로서비스가 하나의 데이터베이스를 지닌다는 것입니다. 이러한 마이크로 서비스의 특징으로 인해, `동시성 제어`, `데이터 동기화`, `데이터의 무결성 보장`와 같은 문제점을 겪고 있습니다. 현재는 데이터베이스간 관련성을 낮추고 단순화하였습니다. 따라서, 현재 프로젝트는 서비스가 서로의 데이터베이스에 접근해야하는 일을 줄였습니다.
